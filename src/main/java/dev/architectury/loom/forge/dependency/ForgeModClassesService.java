...
public final class ForgeModClassesService extends Service<ForgeModClassesService.Options> {
	...
	public static Provider<Options> createOptions(Project project, Provider<ClasspathGroup.ClasspathType> type) {
		return TYPE.maybeCreate(project, options -> {
			...
			for (RunConfigSettings runConfigSettings : extension.getRunConfigs()) {
				...
				options.getClasspathGroupOptions().put(runConfigSettings.getName(), ClasspathGroupService.create(project, modSettings, type));
			}
	...
	public String getModClasses(String runConfig) {
		Multimap<String, String> modClasses = Multimap.listMultimap();
